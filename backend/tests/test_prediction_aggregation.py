...
class PredictionAggregationTests(unittest.TestCase):
    def test_averages_complete_probability_distributions(self):
        entries = [
            accepted_photo(0, "brown_spot", 0.7, {"brown_spot": 0.7, "healthy": 0.3}),
            accepted_photo(1, "healthy", 0.6, {"brown_spot": 0.4, "healthy": 0.6}),
        ...
        self.assertAlmostEqual(result["classificationConfidence"], 0.633333, places=6)
        self.assertEqual(result["consensusCount"], 2)
        self.assertAlmostEqual(result["consensusRatio"], 2 / 3, places=6)
...
class PredictionRouteTests(unittest.TestCase):
    ...
    def test_multi_photo_request_returns_combined_result(self):
        predictions = [
            model_result("brown_spot", {"brown_spot": 0.8, "healthy": 0.2}),
            model_result("healthy", {"brown_spot": 0.6, "healthy": 0.4}),
        ]
        uploads = [
